...
export class TypeConvertor
{
    ...
    convert(type:string, localePath:string) : string
    {
        if (localePath)
        {
            if (localePath.startsWith("@")) localePath = "." + localePath.substring(1);

            if (this.mapper.has(localePath)) return this.mapper.get(localePath);
            if (localePath.indexOf("<") < 0)
            {
                ...
                if (m >= 0)
                {
                    if (this.mapper.has(localePath.substring(m))) return this.mapper.get(localePath.substring(m));
                    var n = localePath.lastIndexOf(".");
                    if (n > m && this.mapper.has(localePath.substring(n))) return this.mapper.get(localePath.substring(n));
                }
            }
        }
